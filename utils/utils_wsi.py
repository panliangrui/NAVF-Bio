...
#
# def load_hdyolo_model(model, device='cpu', nms_params={}):
#     # 如果传入的是字符串，加载 TorchScript 模型
#     if isinstance(model, str):
#         model = torch.jit.load(model, map_location='cpu')
#
#     # 检查 model 是否具有预期的结构
#     if hasattr(model, 'headers') and hasattr(model.headers, 'det') and hasattr(model.headers.det, 'nms_params'):
#         # 更新 nms_params，确保在原始模型中存在这个属性
#         new_nms_params = {
#             k: nms_params.get(k, v)
#             for k, v in model.headers.det.nms_params.items()
#         }
#         model.headers.det.nms_params = new_nms_params
#     else:
#         raise AttributeError("The model does not have the expected 'headers.det.nms_params' structure.")
#
#     # 处理设备参数
#     if isinstance(device, str):
#         device = torch.device(device)
#
#     # 如果使用 CPU，则确保模型是 float32 类型，因为 float16 只支持 CUDA
#     if device.type == 'cpu':
#         model = model.float()
#
#     # 设置模型为评估模式并转移到指定设备
